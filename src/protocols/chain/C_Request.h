...
public:
	C_Request();
	C_Request(C_Request_rep* cont);
	C_Request(Request_id r);
	...
	// Effects: Sets the sequence number for the message.

	void nullify_seqno(Seqno* seq);
	// Effects: sets the sequence number to 0, and returns orignal seqno in seq
	...
	static bool convert(char *m1, unsigned max_len, C_Request *&m2);
...
inline Seqno &C_Request::seqno()
{
   return rep().seq_num;
}

inline void C_Request::set_seqno(Seqno& seq)
{
   rep().seq_num = seq;
}

inline void C_Request::nullify_seqno(Seqno* seq)
{
	if (seq != NULL) {
		*seq = rep().seq_num;
		rep().seq_num = 0;
