...
void C_Checkpoint::re_authenticate(C_Principal *p, bool stable)
{
   if (rep().extra != 1 && stable) {
      rep().extra = 1;
      set_size(sizeof(C_Checkpoint_rep) + C_node->sig_size(id()));
...
bool C_Checkpoint::verify()
{
  ...
  // checkpoint is valid if sent by self
  if (id() == C_node->id()) return true;

  // Check signature size.
  if (size()-(int)sizeof(C_Checkpoint_rep) < (int)C_node->sig_size(id()))
    return false;

  bool ret = C_node->i_to_p(id())->verify_signature(contents(), sizeof(C_Checkpoint_rep), contents()+sizeof(C_Checkpoint_rep));
  return ret;
}
