#ifndef _zl_Message_h
#define _zl_Message_h 1
...
const unsigned zl_Max_message_size = 60000;

//
// All messages have the following format:
//
struct zl_Message_rep
{
	short tag;
	...
	//Followed by char payload[size-sizeof(zl_Message_rep)];
};
...
class zl_Message
{
	...
	//
public:
	zl_Message(unsigned sz=0);
	// Effects: Creates an untagged zl_Message object that can hold up
...
protected:
	...
	zl_Message_rep *msg; // Pointer to the contents of the message.
...
inline unsigned zl_Message::size() const
{
	return msg->size;
}

inline int zl_Message::tag() const
{
	return msg->tag;
}

inline bool zl_Message::has_tag(int t, int sz) const
{
...
inline void* zl_Message::operator new(size_t s)
{
...
inline void zl_Message::operator delete(void *x, size_t s)
{
	if (x != 0)
	{
		free((char*)x);
...
inline unsigned zl_Message::msize() const
{
	return (max_size >= 0) ? max_size : msg->size;
}

inline char *zl_Message::contents()
{
	return (char *)msg;
}
