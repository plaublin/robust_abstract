...
zl_Node::zl_Node(FILE *config_file, FILE *config_priv, char *host_name, short req_port)
{
	...
	for (int i=0; i < num_principals; i++)
	{
		...
		//if (i < num_replicas || ((node_id!=-1) && (node_id<num_replicas)))
...
//	zl_Message::init();
	...
	// name the socket
	// on a single NIC, this will work for both MCAST and unicast.
	// for multihomed systems, there needs to be a separate socket for MCAST,
	// bound to group's address
	Addr tmp;
	...
	// Disable loopback
	u_char l = 0;
	error = setsockopt(sock, IPPROTO_IP, IP_MULTICAST_LOOP, &l, sizeof(l));
	if (error < 0)
	{
...
#undef NO_IP_MULTICAST
#ifdef NO_IP_MULTICAST
	if (i == zl_All_replicas) {
	    for (int j=0; j<num_replicas; j++) {
		int error = 0;
		while (error < (int)m->size())
		{
		    const Addr *to = principals[j]->address();
...
{
    return has_messages(to, sock);
}

bool zl_Node::has_messages(long to, int socket)
{
	...
	FD_ZERO(&fdset);
	FD_SET(socket, &fdset);
	int ret = select(socket+1, &fdset, 0, 0, &timeout);
	if (ret > 0&& FD_ISSET(socket, &fdset))
	{
		return true;
	}
	return false;
...
zl_Message* zl_Node::recv(int socket)
{
	...
	while (1)
	{
...
#endif
		int ret = recvfrom(socket, m->contents(), m->msize(), 0, 0, 0);
		...
		while (!has_messages(20000, socket))
			;
	}
}

zl_Message* zl_Node::recv_tcp(int socket)
{
   zl_Message* m = new zl_Message(zl_Max_message_size);

   recv_all_blocking(socket, (char *)m->contents(), sizeof(zl_Message_rep),
         0);
   int payload_size = m->size() - sizeof(zl_Message_rep);
   recv_all_blocking(socket, m->contents() + sizeof(zl_Message_rep),
         payload_size, 0);
   return m;
}
