...
public:
	...
	// Effects: Returns the id of the instance (zlight)
  ...
  // Effects: Returns the id of the next instance
	...
	zl_Message* recv();
	zl_Message* recv(int socket);
	zl_Message* recv_tcp(int socket);
	...
	bool has_messages(long to, int socket);
...
inline enum protocols_e zl_Node::next_instance_id() const
{
    return chain;
}
