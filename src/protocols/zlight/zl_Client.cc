...
bool zl_Client::send_request(zl_Request *req, int size, bool ro)
{
   ...
   if (out_rid % NOTIF_PERIOD == 0) {
      zl_Client_notification notif(zl_Client_notification_request, id(), req->request_id()); 
      send(&notif, zl_All_replicas);
   }
#endif
...
#if 0
      fprintf(stderr, "zl_Client[%d], gonna add rep %lld from %d\n", id(), rep->request_id(), rep->id());
#endif
...
#ifdef CLIENT_SENDS_NOTIFICATION
            if (out_rid % NOTIF_PERIOD == 0) {
               zl_Client_notification notif(zl_Client_notification_reply, id(), out_req->request_id()); 
               send(&notif, zl_All_replicas);
            }
#endif
