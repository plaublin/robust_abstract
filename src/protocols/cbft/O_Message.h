...
const unsigned O_Max_message_size = 9000;

//
// All messages have the following format:
//
struct O_Message_rep
{
	short tag;
	...
	//Followed by char payload[size-sizeof(O_Message_rep)];
};
...
class O_Message
{
	...
	//
public:
	O_Message(unsigned sz=0);
	// Effects: Creates an untagged O_Message object that can hold up
...
protected:
	...
	O_Message_rep *msg; // Pointer to the contents of the message.
...
inline unsigned O_Message::size() const
{
	return msg->size;
}

inline int O_Message::tag() const
{
	return msg->tag;
}

inline bool O_Message::has_tag(int t, int sz) const
{
...
inline void* O_Message::operator new(size_t s)
{
...
inline void O_Message::operator delete(void *x, size_t s)
{
	if (x != 0)
	{
		free((char*)x);
...
inline unsigned O_Message::msize() const
{
	return (max_size >= 0) ? max_size : msg->size;
}

inline char *O_Message::contents()
{
	return (char *)msg;
}
