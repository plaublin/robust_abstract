#ifndef _Q_Certificate_h
#define _Q_Certificate_h 1
...
template<class T> class O_Certificate
{
   //
...
template<class T> inline T *O_Certificate<T>::cvalue() const
{
   return value;
}

template<class T> inline bool O_Certificate<T>::is_complete() const
{
   //fprintf(stderr, "gags.count = %d\n", count);
   return count >= complete;
}

template<class T> inline void O_Certificate<T>::clear()
{
   count = 0;
   bmap.clear();
   value = 0;
...
template<class T> inline void O_Certificate<T>::set_complete(int c)
{
  complete = c;
}

#endif // O_Certificate_h
