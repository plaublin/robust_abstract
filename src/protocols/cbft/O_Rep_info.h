#ifndef _Q_Rep_info_h
#define _Q_Rep_info_h 1
...
public:
   ...
   void send_reply(int dest_pid, View v, int src_pid, bool tentative=true, int totalOrder=-1);
...
private:
   int nps;
   char *mem;
   O_Reply** reps; // Array of replies indexed by principal id.
   static const int Max_rep_size = 8192;
};

inline int O_Rep_info::size() const
{
   return (nps+1)*Max_rep_size;
}

inline Request_id O_Rep_info::req_id(int pid)
{
   return reps[pid]->request_id();
}

inline Digest& O_Rep_info::digest(int pid)
{
   return reps[pid]->digest();
}

inline O_Reply* O_Rep_info::reply(int pid)
{
   return reps[pid];
}
