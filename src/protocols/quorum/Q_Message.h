...
const unsigned Q_Max_message_size = 60000;

//
// All messages have the following format:
//
struct Q_Message_rep
{
	short tag;
	...
	//Followed by char payload[size-sizeof(Q_Message_rep)];
};
...
class Q_Message
{
	...
	//
public:
	Q_Message(unsigned sz=0);
	// Effects: Creates an untagged Q_Message object that can hold up
...
protected:
	...
	Q_Message_rep *msg; // Pointer to the contents of the message.
...
inline unsigned Q_Message::size() const
{
	return msg->size;
}

inline int Q_Message::tag() const
{
	return msg->tag;
}

inline bool Q_Message::has_tag(int t, int sz) const
{
...
inline unsigned Q_Message::msize() const
{
	return (max_size >= 0) ? max_size : msg->size;
}

inline char *Q_Message::contents()
{
	return (char *)msg;
}
