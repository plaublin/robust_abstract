...
public:
   ...
   long long send_reply(int dest_pid, View v, int src_pid, bool tentative=true);
...
private:
   int nps;
   char *mem;
   Q_Reply** reps; // Array of replies indexed by principal id.
   static const int Max_rep_size = 8192;
};

inline int Q_Rep_info::size() const
{
   return (nps+1)*Max_rep_size;
}

inline Request_id Q_Rep_info::req_id(int pid)
{
   return reps[pid]->request_id();
}

inline Digest& Q_Rep_info::digest(int pid)
{
   return reps[pid]->digest();
}

inline Q_Reply* Q_Rep_info::reply(int pid)
{
   return reps[pid];
}
