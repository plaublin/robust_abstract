...
R_Rep_info::new_reply(int pid, int &sz)
{
  ...
  r->rep().reply_size = -1;
  sz = Max_rep_size - sizeof(R_Reply_rep) - (R_node->f() + 1) * ((R_node->f()+1)*R_UMAC_size + R_UNonce_size);
  return r->contents() + sizeof(R_Reply_rep);
}

R_Reply_rep *
R_Rep_info::end_reply(int pid, int entry_replica, Request_id rid, Seqno seqno, char *data, int sz)
{
  R_Reply* r = reps[pid];
  ...
  memcpy(r->contents()+sizeof(R_Reply_rep), data, sz);
  ...
  r->set_size(old_size + (R_node->f() + 1) * (R_UNonce_size + (R_node->f()+1)*R_UMAC_size));
...
#ifdef USE_MACS
  int macs_size = (R_node->f() + 1) * (R_UNonce_size + (R_node->f()+1)*R_UMAC_size);
  memcpy(r->contents() + old_size, MACs, macs_size);
#endif

  R_node->send(r, socket);
#ifdef USE_MACS
  return old_size + macs_size;
#else
  return old_size;
#endif
