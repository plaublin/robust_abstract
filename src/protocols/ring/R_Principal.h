#ifndef _R_Principal_h
#define _R_Principal_h 1
...
class R_Principal {
public:
	R_Principal(int i, int num_principals, Addr TCP_a, Addr TCP_a_for_clients, char *p=NULL);
	// Effects: Creates a new R_Principal object.
	...
	void gen_mac(const char *src, unsigned src_len, char *dst, int dest_pid,
			const char *unonce);
	// Requires: "dst" can hold at least "MAR_size" bytes.
	// Effects: Generates a MAC (with MAR_size bytes) using the key for dest_pid and
	// places it in "dst".  The MAC authenticates "src_len" bytes
	// starting at "src".
	void gen_mac_partial(const char *src, unsigned src_len, char *dst,
		int dest_pid, const char *unonce);
	void gen_mac_close_partial(int dest_pid);
...
inline int R_Principal::pid() const {
	return id;
}

inline int R_Principal::sig_size() const { return ssize; }

#endif // _R_Principal_h
