#ifndef _PBFT_R_Request_h
#define _PBFT_R_Request_h 1
...
//
// PBFT_R_Request messages have the following format.
//
struct PBFT_R_Request_rep : public PBFT_R_Message_rep
{
		Digest od; // Digest of rid,cid,command.
		short replier; // id of PBFT_R_replica from which client
		// expects to receive a full reply
		// (if negative, it means all PBFT_R_replicas).
...
class PBFT_R_Request : public PBFT_R_Message
{
		//
		// PBFT_R_Request messages:
		//
		// Requires: PBFT_R_Requests that may have been allocated by library users
		// through the pbft_libbyz.h interface can not be trimmed (this could free
	...
	public:
		...
		PBFT_R_Request(Request_id r, short rr=-1);
		// Effects: Creates a new signed PBFT_R_Request message with an empty
		// command and no authentication. The methods store_command and
		// authenticate should be used to finish message construction.
		// "rr" is the identifier of the PBFT_R_replica from which the client
		// expects a full reply (if negative, client expects a full reply
		// from all PBFT_R_replicas).

		PBFT_R_Request(PBFT_R_Request_rep *contents);
		// Requires: "contents" contains a valid PBFT_R_Request_rep. Otherwise, use
		// the static method convert.
		// Effects: Creates a PBFT_R_Request message from "contents". No copy
		// is made of "contents" and the storage associated with "contents"
		// is not deallocated if the message is later deleted.

		PBFT_R_Request* clone() const;
		// Effects: Clones this.
		...
		void re_authenticate(bool change=false, PBFT_R_Principal *p=0);
		...
		int client_id() const;
		// Effects: PBFT_R_Fetches the identifier of the client from the message.
		...
		int replier() const;
		// Effects: Returns the identifier of the PBFT_R_replica from which
		// the client expects a full reply. If negative, client expects
		// a full reply from all PBFT_R_replicas.
		...
		static bool convert(PBFT_R_Message *m1, PBFT_R_Request *&m2);
		// Effects: If "m1" has the right size and tag of a "PBFT_R_Request",
		// casts "m1" to a "PBFT_R_Request" pointer, returns the pointer in
		// "m2" and returns true. Otherwise, it returns false.

		static bool convert(char *m1, unsigned max_len, PBFT_R_Request &m2);
		// Requires: convert can safely read up to "max_len" bytes starting
		// at "m1"
		// Effects: If "m1" has the right size and tag of a
		// "PBFT_R_Request_rep" assigns the corresponding PBFT_R_Request to m2 and
		...
		Digest& digest() const { return rep().od; }

	private:
		PBFT_R_Request_rep &rep() const;
		// Effects: Casts "msg" to a PBFT_R_Request_rep&

		void comp_digest(Digest& d);
		// Effects: computes the digest of rid, cid, and the command.
};

inline PBFT_R_Request_rep& PBFT_R_Request::rep() const
{
	th_assert(ALIGNED(msg), "Improperly aligned pointer");
	return *((PBFT_R_Request_rep*)msg);
}

inline int PBFT_R_Request::client_id() const
{
	return rep().cid;
}

inline Request_id &PBFT_R_Request::request_id()
{
	return rep().rid;
}

inline char *PBFT_R_Request::command(int &len)
{
	len = rep().command_size;
	return contents()+sizeof(PBFT_R_Request_rep);
}

inline int PBFT_R_Request::replier() const
{
	return rep().replier;
}

inline bool PBFT_R_Request::is_read_only() const
{
	return rep().extra & 1;
}

inline bool PBFT_R_Request::is_signed() const
{
	return rep().extra & 2;
}

#endif // _PBFT_R_Request_h
