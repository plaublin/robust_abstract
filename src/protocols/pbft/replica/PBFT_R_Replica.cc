...
//#define DO_TIMINGS
#define _MEASUREMENTS_ID_ (PBFT_R_replica->id())
		...
		PBFT_R_Time *tsent, PBFT_R_Principal *p)
{
	...
	if (1)
	{
		...
		// Retransmit message
		send(m, p->pid());
		UPDATE_OUT(R,POST,m->size());

		*tsent = cur;
	}
}

#if defined (DO_STATISTICS) || defined (DO_TIMINGS)
#include <signal.h>
static void kill_replica(int sig)
{
    ...
    {
    	delete PBFT_R_replica;
		PBFT_R_replica=NULL;
    }

    fprintf(stderr, "REPLICA EXITING\n");
    exit(0);
}
#endif
...
missing(NULL), ah_2(NULL), num_missing(0),
missing_mask(), missing_store(), missing_store_seq(),
aborts(3*f()+1, 3*f()+1), outstanding()
{
         ...
         aborts(3*f()+1, 3*f()+1), outstanding()
{
   ...
   fprintf(stderr, "PBFT_R_Replica: Constructor: creating a new Req_history_log\n");
   rh = new Req_history_log<PBFT_R_Request>();
	...
	my_host_name = (char*)malloc(strlen(host_name)+1);
	strcpy(my_host_name, host_name);
...
void PBFT_R_Replica::recv()
{
	...
	while (1)
	{
		...
		switch (m->tag())
		{
			...
			case PBFT_R_Panic_tag:
				gen_handle<PBFT_R_Panic>(m);
				break;

			case PBFT_R_Abort_tag:
				gen_handle<PBFT_R_Abort>(m);
				break;
...
void PBFT_R_Replica::handle(PBFT_R_Request *m)
{
   ...
   if (cur_state == replica_state_STOP) {
       ...
       PBFT_R_Reply qr(0, m->request_id(), node_id, replies.digest(cid), i_to_p(cid), cid);
       qr.set_instance_id(next_instance_id());
       qr.authenticate(i_to_p(cid), 0, false);
...
void PBFT_R_Replica::handle(PBFT_R_Panic *m)
{
//   fprintf(stderr, "PBFT_R_Replica[%d]: has received a Panic message!!!\n", id());

   int cid = m->client_id();
   Request_id rid = m->request_id();

   //fprintf(stderr, "PBFT_R_Replica[%d]:: receiving panic for (cid = %d rid = %llu)\n", id(), cid, rid);
   if (cur_state != replica_state_PANICKING && cur_state != replica_state_STOP) {
       PBFT_R_Panic qp(id(), 0);
       send(&qp, All_PBFT_R_replicas);
       n_retrans = 0;
       cur_state = replica_state_PANICKING;
       fprintf(stderr, "PBFT_R_Replica[%d](1): cur_state=%d\n", id(), cur_state);
       rtimer->restop();
       rtimer->start();
   }
   ...
   if (!is_PBFT_R_replica(cid)) {
      if (cur_state == replica_state_STOP) {
         PBFT_R_Reply qr(0, rid, node_id, replies.digest(cid), i_to_p(cid), cid);
         qr.set_instance_id(next_instance_id());
         qr.authenticate(i_to_p(cid), 0, false);
...
void PBFT_R_Replica::retransmit_panic()
{
   //fprintf(stderr, "PBFT_R_Replica[%d]: will retransmit PANIC!\n", id());
   ...
   if (n_retrans == nk_thresh) {
      ...
      fprintf(stderr, "PBFT_R_Replica[%d](2): cur_state=%d\n", id(), cur_state);
      great_switcher->switch_state(instance_id(), false);
      great_switcher->switch_state(next_instance_id(), true);
      n_retrans = 0;
      return;
   ...
   send(&qp, All_PBFT_R_replicas);
   cur_state = replica_state_PANICKING;
   fprintf(stderr, "PBFT_R_Replica[%d](3): cur_state=%d\n", id(), cur_state);

   rtimer->restart();
}

void PBFT_R_Replica::broadcast_abort(Request_id out_rid)
{
   //fprintf(stderr, "PBFT_R_Replica[%d]::broadcast_abort for (cid = %d rid = %llu)\n", id(), cid, out_rid);
   PBFT_R_Abort *a_message = new PBFT_R_Abort(node_id, out_rid, *rh);

   a_message->sign();
   send(a_message, All_PBFT_R_replicas);
   // since we're not receiving it back:
   if (!aborts.add(a_message))
      delete a_message;
}

void PBFT_R_Replica::handle(PBFT_R_Abort *m)
{
//   fprintf(stderr, "PBFT_R_Replica[%d]: has received a Abort message!!!\n", id());
   ...
   if (cur_state == replica_state_PANICKING)
   {
      ...
      if (!m->verify())
      {
         fprintf(stderr, "PBFT_R_Replica[%d]: Unable to verify an Abort message\n", id());
         delete m;
         return;
      }

#ifdef TRACE
      fprintf(stderr, "PBFT_R_Replica[%d]: received Abort message from %d (cid = %d, rid = %llu) (hist_size = %d)\n", id(), m->id(), m->client_id(), m->request_id(), m->hist_size());
#endif

      if (!aborts.add(m))
      {
         fprintf(stderr, "PBFT_R_Replica[%d]: Failed to add Abort from %d to the certificate\n", id(), m->id());
         delete m;
         return;
      }
      if (aborts.is_complete())
      {
         ...
         if (missing->size() == 0) {
            ...
            Req_history_log<PBFT_R_Request> *newrh = new Req_history_log<PBFT_R_Request>();
            for (int i=0; i<ah_2->size(); i++) {
               // if it is in rh, just copy it
               PBFT_R_Request *ar = NULL;
               Rh_entry<PBFT_R_Request> *rhe = NULL;
            ...
            cur_state = replica_state_STOP;
            fprintf(stderr, "PBFT_R_Replica[%d](4): cur_state=%d\n", id(), cur_state);
            great_switcher->switch_state(instance_id(), false);
            great_switcher->switch_state(next_instance_id(), true);
         ...
         PBFT_R_Missing qmis(id(), missing);
         send(&qmis, All_PBFT_R_replicas);

         // and then wait to get a grip on these missing request
         cur_state = replica_state_MISWAITING;
         fprintf(stderr, "PBFT_R_Replica[%d](5): cur_state=%d\n", id(), cur_state);
   ...
   {
      // fprintf(stderr, "PBFT_R_Replica[%d]: Receiving an Abort message during Panic mode\n", id());
      delete m;
   }
}

void PBFT_R_Replica::notify_outstanding() {
...
void PBFT_R_Replica::handle(PBFT_R_Missing *m)
{
    // extract the replica
    int replica_id = m->id();
   ...
   Rh_entry<PBFT_R_Request> *rhe = rh->find(cur_cid, cur_rid);
   if (rhe != NULL) {
       // and construct Get-a-grip messages out of request
       PBFT_R_Get_a_grip qgag(cur_cid, cur_rid, id(), rhe->seqno(), (PBFT_R_Request*)rhe->req);
...
void PBFT_R_Replica::handle(PBFT_R_Get_a_grip *m)
{
    if (cur_state != replica_state_MISWAITING) {
      fprintf(stderr, "PBFT_R_Replica[%d]::PBFT_R_get_a_grip: got unneeded PBFT_R_GAG message\n", id());
   delete m;
   return;
    }
    ...
    int rep_size = ((PBFT_R_Message_rep *)m->stored_request())->size;
    //((Reply_rep *)gag->reply())->replica = gag->id();
    void *rep_ = malloc(rep_size);
    memcpy(rep_, m->stored_request(), rep_size);
    PBFT_R_Request *req = new PBFT_R_Request((PBFT_R_Request_rep *)rep_);
...
void PBFT_R_Replica::send_pre_prepare()
{
			...
			&& seqno + 1 <= max_out + last_stable && has_new_view())
	{
		...
		//printf("SND: PRE-PREPARE seqno=%lld last_stable=%lld\n", seqno+1, last_stable);
		...
		if (ms <= last_stable + max_out)
		{
			...
			if (!late)
			{
				...
				if (cs.add(m) && cs.mine() && cs.is_complete())
				{
					...
					//	  fprintf(stderr, "CP MSG call MS %lld!!!\n", last_executed);
					mark_stable(ms, true);
				}
				//	else {
				//	  fprintf(stderr, "CP msg %lld not yet. Reason: ", ms);
				//	  if (!cs.mine())
...
{
	//  printf("RECV: view change v=%lld from %d\n", m->view(), m->id());
...
{
	//  printf("RECV: new view v=%lld from %d\n", m->view(), m->id());
...
{
	//  printf("RECV: view-change ack v=%lld from %d for %d\n", m->view(), m->id(), m->vc_id());
...
void PBFT_R_Replica::send_view_change()
{
	...
	if (last_tentative_execute > last_executed)
	{
		...
		th_assert(!state.in_fetch_state(), "Invalid state");
		Seqno rc = state.rollback();
		//    printf("XXXRolled back in vc to %lld with last_executed=%lld\n", rc, last_executed);
		last_tentative_execute = last_executed = rc;
		//    fprintf(stderr, ":):):):):):):):) Set le = %d\n", last_executed);
	}
	...
	printf("XXX SND: view change %lld\n", v);
...
void PBFT_R_Replica::process_new_view(Seqno min, Digest d, Seqno max, Seqno ms)
{
	...
	//  printf("XXX process new view: %lld\n", v);
	...
	//  printf("XXX DONE:process new view: %lld\n", v);
}
...
bool PBFT_R_Replica::execute_read_only(PBFT_R_Request *req)
{
			...
			&& !state.in_check_state())
	{
		...
		if (j2ee)
			...
			error = exec_command(&inb, &outb, (Byz_buffer*)&last_executed, cid, true);
		...
		if (pp && pp->view() == view())
		{
			...
			while (iter.get(req))
			{
				...
				if (is_PBFT_R_replica(cid))
				{
					...
					//	  printf("\n\n\nExecuting recovery request seqno=%lld rep id=%d\n", last_tentative_execute, cid);
			...
			if (pp && pp->view() == view())
			{
				...
				if (last_executed % checkpoint_interval == 0)
				{
					...
					if (cc.is_complete())
					{
						...
						//	    fprintf(stderr, "EXEC call MS %lld!!!\n", last_executed);
					}
					//	  else
					//	    fprintf(stderr, "CP exec %lld not yet. ", last_executed);
...
void PBFT_R_Replica::mark_stable(Seqno n, bool have_state)
{
	...
	//  fprintf(stderr, "mark stable n %lld laststable %lld\n", n, last_stable);
...
void PBFT_R_Replica::enforce_bound(Seqno b)
{
	...
	if (!correct)
	{
		fprintf(stderr, "Incorrect state setting low bound to %lld\n",
		known_stable);
			...
			|| !m->is_tentative()))
	{
		...
		if (added)
		{
			...
			if (rPBFT_R_reps.is_complete())
			{
				...
				//	printf("XXX Complete rec reply with seqno %lld rec_point=%lld\n",rec_seqno,  recovery_point);
...
{
    fprintf(stderr, "PBFT_R_Replica[%d]: will try to switch state, switch %d, when in state %d\n", id(), state, cur_state);
