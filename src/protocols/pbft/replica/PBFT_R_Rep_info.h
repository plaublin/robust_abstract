#ifndef _PBFT_R_Rep_info_h
#define _PBFT_R_Rep_info_h 1
...
public:
  ...
  PBFT_R_Rep_info(int nps);
  // Effects: Creates a new object that stores data for "nps"
  // principals.
#else
  PBFT_R_Rep_info(char *mem, int sz, int nps);
  ...
  long long send_reply(int pid, View v, int id, bool tentative=true);
  // Requires: "pid" is a valid principal identifier and end_reply was
  // called after the last call to new_reply for "pid"
  // Effects: Sends a reply message to "pid" for view "v" from PBFT_R_replica
...
private:
  int nps;
  char *mem;
  Array<PBFT_R_Reply*> reps; // Array of replies indexed by principal id.
  static const int Max_rep_size = 8192;

  struct Rinfo {
    bool tentative;       // True if last reply is tentative and was not committed.
    PBFT_R_Time lsent;           // PBFT_R_Time at which reply was last sent.
  };
  Array<Rinfo> ireps;
  bool j2ee;
};

inline int PBFT_R_Rep_info::size() const {
  return (nps+1)*Max_rep_size;
}

inline void PBFT_R_Rep_info::commit_reply(int pid) {
  ireps[pid].tentative = false;
  ireps[pid].lsent = zeroPBFT_R_Time();
}

inline bool PBFT_R_Rep_info::is_committed(int pid) {
  return !ireps[pid].tentative;
}

inline Request_id PBFT_R_Rep_info::req_id(int pid) {
  return reps[pid]->request_id();
}

inline Digest& PBFT_R_Rep_info::digest(int pid) {
  return reps[pid]->digest();
}

inline PBFT_R_Reply* PBFT_R_Rep_info::reply(int pid) {
  return reps[pid];
}

#ifndef NO_STATE_TRANSLATION
inline char* PBFT_R_Rep_info::rep_info_mem() {
  return mem;
}
#endif

#endif // _PBFT_R_Rep_info_h
