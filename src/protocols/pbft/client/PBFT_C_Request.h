#ifndef _PBFT_C_Request_h
#define _PBFT_C_Request_h 1
...
//
// PBFT_C_Request messages have the following format.
//
struct PBFT_C_Request_rep : public PBFT_C_Message_rep {
...
class PBFT_C_Request : public PBFT_C_Message {
  //
  // PBFT_C_Request messages:
  //
  // Requires: PBFT_C_Requests that may have been allocated by library users
  // through the pbft_libbyz.h interface can not be trimmed (this could free
  // memory the user expects to be able to use.)
  //
public:
  PBFT_C_Request() : PBFT_C_Message() {}

  PBFT_C_Request(Request_id r, short rr=-1);
  // Effects: Creates a new signed PBFT_C_Request message with an empty
  ...
  PBFT_C_Request(PBFT_C_Request_rep *contents);
  // Requires: "contents" contains a valid PBFT_C_Request_rep. Otherwise, use
  // the static method convert.
  // Effects: Creates a PBFT_C_Request message from "contents". No copy
  // is made of "contents" and the storage associated with "contents"
  // is not deallocated if the message is later deleted.

  PBFT_C_Request* clone() const;
  // Effects: Clones this.
  ...
  void re_authenticate(bool change=false, PBFT_C_Principal *p=0);
  ...
  int client_id() const;
  // Effects: PBFT_C_Fetches the identifier of the client from the message.
  ...
  static bool convert(PBFT_C_Message *m1, PBFT_C_Request *&m2);
  // Effects: If "m1" has the right size and tag of a "PBFT_C_Request",
  // casts "m1" to a "PBFT_C_Request" pointer, returns the pointer in
  // "m2" and returns true. Otherwise, it returns false.

  static bool convert(char *m1, unsigned max_len, PBFT_C_Request &m2);
  // Requires: convert can safely read up to "max_len" bytes starting
  // at "m1"
  // Effects: If "m1" has the right size and tag of a
  // "PBFT_C_Request_rep" assigns the corresponding PBFT_C_Request to m2 and
...
private:
  PBFT_C_Request_rep &rep() const;
  // Effects: Casts "msg" to a PBFT_C_Request_rep&
...
inline PBFT_C_Request_rep& PBFT_C_Request::rep() const {
  th_assert(ALIGNED(msg), "Improperly aligned pointer");
  return *((PBFT_C_Request_rep*)msg);
}


inline int PBFT_C_Request::client_id() const { return rep().cid; }


inline Request_id &PBFT_C_Request::request_id() { return rep().rid; }


inline char *PBFT_C_Request::command(int &len) {
  len = rep().command_size;
  return contents()+sizeof(PBFT_C_Request_rep);
}


inline int PBFT_C_Request::replier() const { return rep().replier; }


inline bool PBFT_C_Request::is_read_only() const { return rep().extra & 1; }


inline bool PBFT_C_Request::is_signed() const { return rep().extra & 2; }

#endif // _PBFT_C_Request_h
