...
PBFT_C_Principal::PBFT_C_Principal(int i, Addr a, char *p) {
  id = i;
  addr = a;
  last_fetch = 0;
  ...
  tstamp = 0;
  my_tstamp = zeroPBFT_C_Time();
}


PBFT_C_Principal::~PBFT_C_Principal() {
  delete pkey;
}

void PBFT_C_Principal::set_in_key(const unsigned *k) {
...
bool PBFT_C_Principal::verify_mac(const char *src, unsigned src_len,
...
void PBFT_C_Principal::gen_mac(const char *src, unsigned src_len,
			    char *dst, unsigned *k) {
  MD5_CTX context;
  unsigned int digest[4];
...
long long PBFT_C_Principal::umac_nonce = 0;
...
void PBFT_C_Principal::set_out_key(unsigned *k, ULong t) {
  if (t > tstamp) {
    ...
    tstamp = t;
    my_tstamp = currentPBFT_C_Time();
  }
}


bool PBFT_C_Principal::verify_signature(const char *src, unsigned src_len,
				 const char *sig, bool allow_self) {
  // PBFT_C_Principal never verifies its own authenticator.
  if ((id == PBFT_C_node->id()) && !allow_self) return false;
...
unsigned PBFT_C_Principal::encrypt(const char *src, unsigned src_len, char *dst,
			    unsigned dst_len) {
