...
PBFT_C_Node *PBFT_C_node = 0;

//Enable statistics
#include "PBFT_C_Statistics.h"

PBFT_C_Node::PBFT_C_Node(FILE *config_file, FILE *config_priv, short req_port)
{
	PBFT_C_node = this;
	...
	group = new PBFT_C_Principal(num_principals + 1, a);
	...
	principals = (PBFT_C_Principal**) malloc(num_principals * sizeof(PBFT_C_Principal*));
	for (int i = 0; i < num_principals; i++)
	{
		...
		a.sin_addr.s_addr = inet_addr(addr_buff);
		a.sin_port = htons(port);
		principals[i] = new PBFT_C_Principal(i, a, pk);
...
#define ASYNC_SOCK
	...
	last_new_key = 0;
	atimer = new PBFT_C_ITimer(at, PBFT_C_atimer_handler);
}

PBFT_C_Node::~PBFT_C_Node()
{
	for (int i = 0; i < num_principals; i++)
		delete principals[i];
...
void PBFT_C_Node::send(PBFT_C_Message *m, int i)
{
	...
	while (error < size)
	{
		INCR_OP(num_sendto); INCR_CNT(bytes_out,size); START_CC(sendto_cycles);
		...
		perror("PBFT_C_Node::send: sendto");
#endif
	}
}

bool PBFT_C_Node::has_messages(long to)
{
	START_CC(handle_timeouts_cycles);
	PBFT_C_ITimer::handle_timeouts();
	STOP_CC(handle_timeouts_cycles);

	// PBFT_C_Timeout period for select. It puts a lower bound on the timeout
	...
	{
		STOP_CC(select_cycles); INCR_OP(select_success);
		return true;
	} STOP_CC(select_cycles); INCR_OP(select_PBFT_C_fail);
	return false;
}

PBFT_C_Message* PBFT_C_Node::recv()
{
	PBFT_C_Message* m = new PBFT_C_Message(Max_message_size);
	...
	while (1)
	{
		...
		INCR_OP(num_recvfrom); START_CC(recvfrom_cycles);
...
//		fprintf(stderr, "%d recv returned... (ret = %d size = %d)\n", id(), ret, m->size());
		...
		if (ret >= (int) sizeof(PBFT_C_Message_rep) && ret >= m->size())
		{
#ifdef ASYNC_SOCK
			PBFT_C_ITimer::handle_timeouts();
...
void PBFT_C_Node::gen_auth(char *s, unsigned l, bool in, char *dest) const
{
	INCR_OP(num_gen_auth); START_CC(gen_auth_cycles);
...
#else
	long long unonce = PBFT_C_Principal::new_umac_nonce();
...
bool PBFT_C_Node::verify_auth(int i, char *s, unsigned l, bool in, char *dest) const
{
	th_assert(node_id < num_replicas, "Called by non-replica");

	INCR_OP(num_ver_auth); START_CC(ver_auth_cycles);

	PBFT_C_Principal *p = i_to_p(i);

	// PBFT_C_Principal never verifies its own authenticator.
	if (p != 0 && i != node_id)
	{
...
void PBFT_C_Node::gen_signature(const char *src, unsigned src_len, char *sig)
{
	INCR_OP(num_sig_gen); START_CC(sig_gen_cycles);
...
unsigned PBFT_C_Node::decrypt(char *src, unsigned src_len, char *dst, unsigned dst_len)
{
...
Request_id PBFT_C_Node::new_rid()
{
...
void PBFT_C_atimer_handler()
{
	th_assert(PBFT_C_node, "replica is not initialized\n");

	// Multicast new key to all replicas.
	PBFT_C_node->send_new_key();
}

void PBFT_C_Node::send_new_key()
{
	delete last_new_key;

	// Multicast new key to all replicas.
	last_new_key = new PBFT_C_New_key();
