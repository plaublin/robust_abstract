...
A_Meta_data_cert::A_Meta_data_cert() {
  last_mdds = new A_Meta_data_d*[A_node->n()];
  last_stables = new Seqno[A_node->n()];
  ...
  max_size = A_node->n()*(max_out/checkpoint_interval+1);
...
bool A_Meta_data_cert::add(A_Meta_data_d* m, bool mine) {
  if (mine) has_my_message = true;

  if (mine || m->verify()) {
    th_assert(mine || m->id() != A_node->id(), 
	      "verify should return false for messages from self");

    // Check if A_node already had a message in the certificate.
    const int id = m->id();
    A_Meta_data_d* om = last_mdds[id];
    ...
    if (m->last_stable() > ls)
      ls = K_max<Seqno>(A_node->f()+1, last_stables, A_node->n(), Seqno_max);
