#ifndef _A_Pre_prepare_info_h
#define _A_Pre_prepare_info_h 1

#include "types.h"
#include "A_Pre_prepare.h"

class A_Pre_prepare_info {
  //
...
public:
  ...
  void add(A_Pre_prepare* p);
  // Effects: Adds "p" to this.

  void add_complete(A_Pre_prepare* p);
  ...
  class BRS_iter {
    ...
    // in mrmap.
  public:
    BRS_iter(A_Pre_prepare_info const *p, BR_map m);
    // Effects: Return an iterator for the missing requests in mrmap
    
    bool get(A_Request*& r);
...
inline A_Pre_prepare_info::A_Pre_prepare_info() {
  pp = 0;
}


inline void A_Pre_prepare_info::zero() {
  pp = 0;
}


inline void A_Pre_prepare_info::add_complete(A_Pre_prepare* p) {
...
inline A_Pre_prepare* A_Pre_prepare_info::pre_prepare() const {
  return pp;
}


inline BR_map A_Pre_prepare_info::missing_reqs() const {
  return mrmap;
}


inline void A_Pre_prepare_info::clear() {
  delete pp;
  pp = 0;
}


inline bool A_Pre_prepare_info::is_complete() const {
  return pp != 0 && mreqs == 0;
}
