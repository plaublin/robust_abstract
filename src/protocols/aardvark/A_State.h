...
public:
  ...
  A_State(A_Replica *replica, char *memory, int num_bytes);
  ...
  // Effects: Returns true iff the A_replica is fetching missing state.
  ...
  bool restart(FILE* i, A_Replica *rep, Seqno ls, Seqno le, bool corrupt); 
  ...
  bool handle(A_Fetch *m, Seqno last_stable);
...
private:
  ...
  A_DSum* stree[PLevels-1];      // Tree of sums of digests of subpartitions.

  Log<Checkpoint_rec> clog;   // A_Checkpoint log
  ...
  //
  bool fetching;      // true iff A_replica is fetching missing state
  ...
  A_Meta_data_cert* cert;  // certificate for partition we are working on
  int lreplier;          // id of last A_replica we chose as replier
  A_Time last_fetch_t;     // A_Time when last fetch was sent. 


  //
  // Information used while checking state during recovery
  //
  bool checking;       // true iff A_replica is checking state
  Seqno check_start;   // last checkpoint sequence number when checking started
  bool corrupt;        // true iff A_replica's state is known to be corrupt
  ...
  bool check_digest(Digest& d, A_Meta_data* m);
...
inline bool A_State::in_fetch_state() const { return fetching; }

inline bool A_State::retrans_fetch(A_Time cur) const { 
  return fetching && A_diffTime(cur, last_fetch_t) > 100000;
}

inline bool A_State::retrans_fetch() const { 
  return fetching;
}
