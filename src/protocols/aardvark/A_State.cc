...
// bits.
unsigned long *_A_Byz_cow_bits = 0;
char *_A_Byz_mem = 0;
...
public:
  ...
  bool is_cleared();
  // Effects: Returns true iff A_Checkpoint record is not in use.
...
struct A_DSum
{
  ...
  static A_DSum* M; // Modulus for sums must be at most nbits-1 long.
  ...
  inline A_DSum(A_DSum const & other)
  {
    memcpy(sum, other.sum, nbytes);
  }

  inline A_DSum& operator=(A_DSum const &other)
  {
    if (this == &other)
      return *this;
    memcpy(sum, other.sum, nbytes);
...
//
// A_PageCache: LRU cache of copies of objects at the last checkpoint taken
...
public:
  ...
  {
    lru = mru = NULL;
    parts.predict(MaxElems);
  }
  // Effects: Creates an empty A_PageCache record.
...
A_PageCache::~A_PageCache()
{
  clear();
}

inline void A_PageCache::append(int i, BlockCopy *b)
{
  BlockCopy *tmp;
  ...
  { /* The obj was not in the cache. If full evict someone */
...
inline char* A_PageCache::get_obj(int i)
{
  BlockCopy *b;
  if (parts.find(i, b))
    return b->data.data;
...
inline BlockCopy* A_PageCache::fetch_and_remove(int i)
{
  BlockCopy* b;
  if (parts.remove(i, b))
  {
...
inline int A_PageCache::num_entries() const
{
  return parts.size();
}

void A_PageCache::print()
{
...
inline void A_DSum::add(Digest &d)
{
...
void A_DSum::sub(Digest &d)
{
  ...
  {
    ret = mpn_add((mp_ptr) sum, (mp_srcptr) sum, nlimbs, (mp_srcptr) M->sum,
        nlimbs);
...
A_State::A_State(A_Replica *rep, char *memory, int num_bytes) :
A_replica(rep), mem((Block*)memory), nb(num_bytes/Block_size),
cowb(nb), clog(max_out*2, 0), lc(0), last_fetch_t()
{
#else
A_State::A_State(A_Replica *rep, char *memory, int num_bytes) :
  replica(rep), mem((Block*) memory), nb(num_bytes / Block_size), cowb(nb),
      clog(max_out * 2, 0), lc(0), last_fetch_t(0)
{
  ...
  {
    stree[i] = new A_DSum[PLevelSize[i]];
  }

  fetching = false;
  cert = new A_Meta_data_cert;
  lreplier = 0;
...
void A_State::cow(char *m, int size)
{
...
void A_State::digest(Digest& d, int i, Seqno lm, char *data, int size)
{
...
inline int A_State::digest(Digest& d, int l, int i)
{
  char *data;
  int size;
...
void A_State::compute_full_digest()
{
  A_Cycle_counter cc;
  cc.start();
  int np = nb;
...
void A_State::update_ptree(Seqno n)
{
  Bitmap* mods[PLevels];
  ...
  for (int l = PLevels - 1; l > 0; l--)
  {
    ...
    {
      Part& p = ptree[l][i];
      A_DSum& psum = stree[l - 1][i / PSize[l]];
      if (l < PLevels - 1)
      {
...
void A_State::checkpoint(Seqno seqno)
{
...
bool A_State::digest(Seqno n, Digest &d)
{
...
void A_State::discard_checkpoint(Seqno seqno, Seqno le)
{
...
char* A_State::get_data(Seqno c, int i)
{
...
Part& A_State::get_meta_data(Seqno c, int l, int i)
{
...
void A_State::start_fetch(Seqno le, Seqno c, Digest *cd, bool stable)
{
...
void A_State::send_fetch(bool change_replier)
{
  //START_CC(fetch_cycles);

  last_fetch_t = A_currentTime();
...
#endif // PRINT_STATS
  // Send fetch to all. 
  A_Fetch f(rid, p.lu, flevel, p.index, p.c, replier);
  replica->send(&f, A_Node::All_replicas);
  ...
  if (!cert->has_mine())
  {
    ...
    {
      // Add my A_Meta_data_d message to the certificate
      A_Meta_data_d* mdd = new A_Meta_data_d(rid, flevel, p.index, ls);

      for (Seqno n = ls; n <= lc; n += checkpoint_interval)
      {
        if (clog.fetch(n).sd.is_zero())
          continue; //XXXXfind a better way to do this
...
bool A_State::handle(A_Fetch *m, Seqno ls)
{
  bool verified = true;

  if (m->verify())
  {
    A_Principal* pi = replica->i_to_p(m->id());
    int l = m->level();
    int i = m->index();
    ...
    if (pi->last_fetch_rid() < m->request_id() && (l < PLevels - 1 || i < nb))
    {
      ...
      if (rc >= 0 && m->replier() == replica->id())
      {
        ...
        {
          // A_Replica has the requested checkpoint
          chosen = rc;
        }
        else if (lc >= rc && ptree[l][i].lm <= rc)
        {
          // A_Replica's last checkpoint has same value as requested
        ...
        if (chosen >= 0)
        {
          if (l == PLevels - 1)
          {
            ...
            A_Data d(i, p.lm, get_data(chosen, i));
            replica->send(&d, m->id());
          ...
          else
          {
            // Send meta-data
            Part& p = get_meta_data(chosen, l, i);
            A_Meta_data md(m->request_id(), l, i, chosen, p.lm, p.d);
      ...
      {
        // Send meta-data-d
        A_Meta_data_d mdd(m->request_id(), l, i, ls);
...
bool A_State::check_digest(Digest& d, A_Meta_data* m)
{
  th_assert(m->level() < PLevels-1, "Invalid argument");

  int l = m->level();
  int i = m->index();
  A_DSum sum = stree[l][i];
  A_Meta_data::Sub_parts_iter miter(m);
  Digest dp;
  int ip;
  ...
  digest(dp, i, m->last_mod(), (char*) (sum.sum), A_DSum::nbytes);
  return d == dp;
}

void A_State::handle(A_Meta_data* m)
{
  ...
  if (fetching && flevel < PLevels - 1 && m->request_id() == crid && flevel
      == m->level())
  {
    ...
    if (wp.index == m->index() && wp.c >= 0 && m->digest() == wp.d)
    {
      ...
      if (m->verify() && check_digest(wp.d, m))
      {
        ...
        A_Meta_data::Sub_parts_iter iter(m);
        Digest d;
        int index;
...
void A_State::handle(A_Meta_data_d* m)
{
  ...
  if (fetching && m->request_id() == crid && flevel == m->level())
  {
    ...
    if (wp.index == m->index() && m->last_stable() >= lc && m->last_stable()
        >= wp.lu)
    {
      ...
      if (cert->add(m))
      {
        ...
        if (cert->cvalue(cc, cd))
        {
          ...
          {
            // A_State is up-to-date
            if (refetch_level == PLevels && ptree[flevel][wp.index].lm
                <= check_start)
            {
              to_check->_enlarge_by(1);
              CPart &cp = to_check->high();
...
// A_State checking:
//
void A_State::start_check(Seqno le)
{
  checking = true;
  refetch_level = PLevels;
  lchecked = -1;
...
inline bool A_State::check_data(int i)
{
...
void A_State::mark_stale()
{
  cert->clear();
}

bool A_State::shutdown(FILE* o, Seqno ls)
{
...
bool A_State::restart(FILE* in, A_Replica *rep, Seqno ls, Seqno le, bool corrupt)
{
  replica = rep;
...
bool A_State::enforce_bound(Seqno b, Seqno ks, bool corrupt)
{
  bool ret = true;
  for (int i = 0; i < PLevels; i++)
  {
