...
// 
// A_Status messages have the following format:
//
...
struct A_Status_rep : public A_Message_rep {
  View v;                // A_Replica's current view
...
class A_Status : public A_Message {
  // 
  //  A_Status messages
  //
public:
  A_Status(View v, Seqno ls, Seqno le, bool hnvi, bool hnvm);
  // Effects: Creates a new unauthenticated A_Status message.  "v"
  // should be the sending A_replica's current view, "ls" should be the
  // sequence number of the last stable checkpoint, "le" the sequence
  // number of the last request executed, "hnvi" should be true iff
  // the sending A_replica has the complete information for view "v", 
  // and "hnvm" should be true iff the sending A_replica has a 
  ...
  void mark_vcs(int i);
  // Requires: !has_nv_info()
  // Effects: Marks the view-change message from A_replica "i" for
  ...
  // message from A_replica "id" for "view()"
  ...
  static bool convert(A_Message *m1, A_Status *&m2);
  // Effects: If "m1" has the right size and tag of a "A_Status",
  // casts "m1" to a "A_Status" pointer, returns the pointer in
  // "m2" and returns true. Otherwise, it returns false.
  // If the conversion is successful it trims excess allocation.

private:
  A_Status_rep& rep() const;
  // Effects: Casts "msg" to a A_Status_rep&
...
inline A_Status_rep& A_Status::rep() const { 
  return *((A_Status_rep*)msg); 
}


inline char* A_Status::prepared() { 
  char *ret = contents()+sizeof(A_Status_rep);
  return ret; 
}


inline char* A_Status::committed() {
  char* ret =  prepared()+rep().sz;
  return ret;
}


inline BR_info* A_Status::breqs() {
  BR_info* ret = (BR_info*)(committed()+rep().sz);
  return ret;
}
  

inline char* A_Status::vcs() {
  return prepared();
}


inline  PP_info* A_Status::pps() {
  PP_info* ret = (PP_info*)(vcs()+A_Status_rep::vcs_size);
  return ret;			    
}


inline void A_Status::mark(Seqno n, char *set) {
...
inline bool A_Status::test(Seqno n, char *set) {
  int offset = n - last_executed();
  if (offset > 0 && offset <= max_out) 
...
inline void A_Status::mark_prepared(Seqno n) { 
  th_assert(has_nv_info(), "Invalid state");
  mark(n, prepared()); 
}


inline void  A_Status::mark_committed(Seqno n) { 
  th_assert(has_nv_info(), "Invalid state");
  mark(n, committed()); 
}


inline void A_Status::add_breqs(Seqno n, BR_map brs) {
  th_assert(has_nv_info(), "Invalid state");
  th_assert(n > last_executed() 
	    && n <= last_stable() + max_out, "Invalid arguments");
  th_assert((char*)(breqs()+rep().brsz) < contents()+A_Max_message_size, 
            "Message too small");
...
inline int A_Status::id() const { return rep().id; }


inline View A_Status::view() const { return rep().v; }


inline bool A_Status::has_nv_m() const { return rep().extra & 2; }


inline bool A_Status::has_nv_info() const { return rep().extra & 1; }


inline Seqno A_Status::last_stable() const { return rep().ls; }       


inline Seqno A_Status::last_executed() const { return rep().le; }         


inline bool A_Status::is_prepared(Seqno n) {
  th_assert(has_nv_info(), "Invalid state");
  return test(n, prepared()); 
}


inline bool A_Status::is_committed(Seqno n) { 
  th_assert(has_nv_info(), "Invalid state");
  return test(n, committed()); 
}


inline bool A_Status::has_vc(int i) {
  th_assert(!has_nv_info(), "Invalid state");
