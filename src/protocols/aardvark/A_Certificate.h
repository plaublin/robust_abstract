#ifndef _A_Certificate_h
#define _A_Certificate_h 1
...
template<class T> class A_Certificate
{
  //
...
private:
  ...
  A_Time t_sent; // time at which mym was last sent
...
template<class T>
inline T *A_Certificate<T>::mine(A_Time **t)
{
  if (t && mym)
    *t = &t_sent;
  return mym;
...
template<class T>
inline T *A_Certificate<T>::cvalue() const
{
  return (c) ? c->m : 0;
}

template<class T>
inline int A_Certificate<T>::num_correct() const
{
  return (c) ? c->count : 0;
}

template<class T>
inline int A_Certificate<T>::num_complete() const
{
  return complete;
}

template<class T>
inline bool A_Certificate<T>::is_complete() const
{
  return num_correct() >= complete;
}

template<class T>
inline void A_Certificate<T>::make_complete()
{
  if (c)
  {
    c->count = complete;
...
template<class T>
inline bool A_Certificate<T>::is_empty() const
{
  return bmap.all_zero();
}

template<class T>
inline void A_Certificate<T>::clear()
{
...
template<class T>
inline A_Certificate<T>::Val_iter::Val_iter(A_Certificate<T>* c)
{
  cert = c;
  next = 0;
}
...
template<class T>
inline bool A_Certificate<T>::Val_iter::get(T*& m, int& count)
{
