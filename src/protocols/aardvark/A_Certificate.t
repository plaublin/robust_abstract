...
template <class T> 
A_Certificate<T>::A_Certificate(int comp) : bmap(Max_num_replicas) {
   max_size = A_node->f()+1;
   vals = new Message_val[max_size];
   cur_size = 0;
   correct = A_node->f()+1;
   complete = (comp == 0) ? A_node->f()*2+1 : comp;
   c = 0;
   mym = 0; 
}

template <class T> 
A_Certificate<T>::~A_Certificate() {
   delete [] vals;
}

template <class T> 
bool A_Certificate<T>::add(T *m) {
   const int id = m->id();
   if (A_node->is_replica(id) && !bmap.test(id)) {
      ...
      if ((c == 0 || (c->count < complete && c->m->match(m))) && m->verify()) {
         ...
         th_assert(id != A_node->id(), "verify should return false for messages from self");
...
template <class T> 
bool A_Certificate<T>::add_mine(T *m) {
   th_assert(m->id() == A_node->id(), "Invalid argument");
...
template <class T> 
void A_Certificate<T>::mark_stale() {
   if (!is_complete()) {
      int i = 0;
...
template <class T> 
T * A_Certificate<T>::cvalue_clear() {
   if (c == 0) {
      return 0;
   }
...
template <class T> 
bool A_Certificate<T>::encode(FILE* o) {
...
template <class T> 
bool A_Certificate<T>::decode(FILE* in) {
   ...
   for (int i=0; i < cur_size; i++) {
      ...
      if (vals[i].count < 0 || vals[i].count > A_node->n())
         return false;

      if (vals[i].count) {
         vals[i].m = (T*)new A_Message;
