#ifndef _A_Big_req_table_h
#define _A_Big_req_table_h 1
...
class A_Big_req_table {
  ...
  // whose size is greater than A_Request::big_req_thresh.)
  //
public:
  A_Big_req_table();
  ...
  void add_pre_prepare(A_Request* r, Seqno n, View v);
  ...
  bool add_request(A_Request* r, bool verified=true);
  // Requires: r->size() > A_Request::big_req_thresh & verified == r->verify() 
...
private:
  bool check_pcerts(A_BR_entry* bre);
  // Requires: A_replica->has_new_view()
  // Effects: Returns true iff there is some pre-prepare in
  // bre->waiting that has f matching prepares in its prepared
  // certificate.

  void remove_unmatched(A_BR_entry* bre);
  // Requires: bre->r != 0
  // Effects: Zeros entry in unmatched.

  Map<Digest,A_BR_entry*> breqs;
  int max_entries;  // Maximum number of entries allowed in the table.
  Array<A_Request*> unmatched; // Array of requests that have no waiting pre-prepares
                             // indexed by client id.
};

#endif // _A_Big_req_table_h
