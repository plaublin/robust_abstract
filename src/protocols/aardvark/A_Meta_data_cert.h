#ifndef _A_Meta_data_cert_h
#define _A_Meta_data_cert_h 1
...
class A_Meta_data_cert {
  //
  // A A_Meta_data_cert is a set of "matching" meta-data-d messages from
  // different replicas.
  //
  
public:
  A_Meta_data_cert();
  // Effects: Creates an empty A_Meta_data_cert. 

  ~A_Meta_data_cert();
  // Effects: Deletes A_Meta_data_cert and all the messages it contains.
 
  bool add(A_Meta_data_d *m, bool mine=false);
  // Effects: Adds "m" to the A_Meta_data_cert and returns true provided
  // "m" satisfies:
  ...
  void clear();
  // Effects: Discards all messages in A_Meta_data_cert
  
private:
  Seqno *last_stables;   // Array with last last_stables in messages 
                         // sent by each A_replica
  Seqno ls;

  A_Meta_data_d **last_mdds; // Array with the last messages sent by each 
                           // A_replica.
  class Part_val {
  public:
    Digest d;
  ...
  bool has_my_message; // True iff A_replica's message is in this
};

inline bool A_Meta_data_cert::has_mine() const { return has_my_message; }
  
inline bool A_Meta_data_cert::cvalue(Seqno& ci, Digest& di) const {
  if (c < 0)
    return false;
...
inline Seqno A_Meta_data_cert::last_stable() const { return ls; }

#endif // A_Meta_data_cert_h
