...
public:
  ...
  A_View_change *mark_stale(int id);
  ...
  A_New_view *new_view(A_Time **t = 0);
  // Effects: If there is a new-view message stored in "this"
  // returns it. Otherwise, returns 0.

  A_View_change *view_change(int id);
  // Effects: If there is any view-change message from A_replica "id"
  ...
  bool add(A_New_view* nv, A_View_info *parent);
  // Requires: "nv.verify() || A_node->id() == A_node->primary(nv->view())"
  ...
  bool add(A_View_change* vc, bool verified);
  ...
  bool add(A_View_change_ack* vca);
...
private:
  ...
  //
  // A_Data structures to check the correctness and completness of
  // new-view messages.
  //
  struct Ckpt_sum
  {
    ...
    int n_le;
    Seqno max_seqno;
    int id; // identifier of first A_replica proposing this.
  };
  ...
  void get_proofs(Req_sum& cur, A_View_change *vc, Seqno i);
...
inline View A_NV_info::view() const
{
  return v;
}

inline bool A_NV_info::complete() const
{
  return is_complete;
}

inline A_New_view *A_NV_info::new_view(A_Time **t)
{
  if (t && nv)
    *t = &nv_sent;
  return nv;
...
inline A_View_change *A_NV_info::view_change(int id)
{
  return vcs[id].vc;
}
