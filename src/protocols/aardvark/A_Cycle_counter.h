#ifndef _A_Cycle_counter_h
#define _A_Cycle_counter_h 1

/****************** rdtsc() related ******************/

static inline long long A_rdtsc(void) {
  union {
    struct {
...
private:
  ...
  // after: 
  // A_Cycle_counter c; c.start(); c.stop(); 
  //
...
inline void A_Cycle_counter::reset() { 
  accumulated = 0; 
  running = false;
  max_incr = 0;
}
...
inline A_Cycle_counter::A_Cycle_counter() { 
  reset();
}


inline void A_Cycle_counter::start() {
  if (!running) {
    running = true;
    c0 = A_rdtsc();
  }
...
inline void A_Cycle_counter::stop() {
  if (running) {
    running = false;
    c1 = A_rdtsc();
...
inline long long A_Cycle_counter::elapsed() {
  if (running) {
    c1 = A_rdtsc();
...
inline long long A_Cycle_counter::max_increment() {
  return max_incr;
}

#endif // _A_Cycle_counter_h
