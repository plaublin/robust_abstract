...
do
	...
	then
		...
		then
			...
			echo -e "./server $port_quorum $port_backup $port_chain $REPLY_SIZE ../$CONFIG_QUORUM ../$CONFIG_PBFT ../$CONFIG_PRIVATE ../$CONFIG_CHAIN"
	        ssh -n ${LOGIN_NAME}@$node "cd BFT/src/benchmarks ; ./server $node $port_quorum $port_backup $port_chain $REPLY_SIZE ../$CONFIG_QUORUM ../$CONFIG_PBFT ../$CONFIG_PRIVATE ../$CONFIG_CHAIN" &
		elif [ $CLIENT_COUNTER -lt $NUM_CLIENTS ] 
		then
				let "CLIENT_COUNTER+=1"  
				echo "Deploying client $CLIENT_COUNTER on [$node,$port_quorum]" 
				echo -e "./thr_client $port_quorum $port_backup $port_chain $NUM_BURSTS $NUM_MESSAGES_PER_BURST $REQUEST_SIZE ../$CONFIG_QUORUM ../$CONFIG_PBFT ../$CONFIG_CHAIN ../$CONFIG_PRIVATE"
		        ssh -n ${LOGIN_NAME}@$node "cd BFT/src/benchmarks ; ./thr_client $node $port_quorum $port_backup $port_chain $NUM_BURSTS $NUM_MESSAGES_PER_BURST $REQUEST_SIZE ../$CONFIG_QUORUM ../$CONFIG_PBFT ../$CONFIG_CHAIN ../$CONFIG_PRIVATE" &
		fi
	fi
done < $CONFIG_QUORUM
